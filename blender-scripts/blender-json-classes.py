...
try:
    ...
    for ob in bpy.data.objects:
        ...
        bMeshData = {
            "name":mesh.name,
            "vertices":bMeshVertices,
            "normals":bMeshNormals,
            "triangles":bMeshTriangles
        }

        # get vertex and normal data
        for v in mesh.vertices:
            bMeshVertices[v.index] = [v.co[0], v.co[1], v.co[2]] # Add Vertex List
            bMeshNormals["normals"][v.index] = [v.normal[0], v.normal[1], v.normal[2]] # Add Normal List

        # get triangle vertex indices.
        for tri in mesh.loop_triangles:
            bMeshTriangles[tri.index] = [tri.vertices[0],tri.vertices[1],tri.vertices[2]]
    ...
    with open(jsonDataDir, "w") as f:
        json.dump(bMeshData, f,ensure_ascii=False,indent=4 )
